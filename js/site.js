...
const NAV_HTML = `
  <nav id="nav">
    <div class="nav-side">
      ...
      <span class="nav-loc">UTTAR&nbsp;PRADESH,&nbsp;IN</span>
    </div>

    <div class="nav-pill" id="nav-pill">
      <div class="nav-id">
        <a class="nav-avatar" href="/" aria-label="Darshita Gupta — home">
          <img src="/assets/PFP.webp" alt="Darshita Gupta">
        </a>
      </div>
...
const MENU_HTML = `
  <div class="menu-overlay" id="menu-overlay" role="dialog" aria-modal="true" aria-label="Menu">
    ...
    <p class="menu-footer">UTTAR PRADESH, IN</p>
  </div>
`;
...
const FOOTER_HTML = `
  <footer class="footer" id="contact">
    ...
    <p class="ft-credit">Designed by Darshita Gupta · Uttar Pradesh, India @2026</p>
    <nav class="ft-social" aria-label="Social links">
      <a href="https://www.linkedin.com/in/darshita-gupta-a6b21a436?utm_source=share_via&utm_content=profile&utm_medium=member_android" target="_blank" rel="noopener">LinkedIn</a>
      <a href="mailto:hello@darshitagupta.in">Email</a>
      <a href="https://www.behance.net/darshitagupta2" target="_blank" rel="noopener">Behance</a>
    </nav>
  </footer>
`;

const CONTACT_EMAIL = 'hello@darshitagupta.in';
...
function initNavContrast() {
    ...
    function update() {
        ...
        let sum = 0,
            n = 0;
        xs.forEach(x => {
            const l = lumAt(Math.max(1, Math.min(x, innerWidth - 1)), y);
            sum += l;
            n++;
        });
...
function initFooterGrow() {
    ...
    if (reduce) {
        head.style.setProperty('--ft-grow', '1');
        return;
    }
    ...
    function update() {
        ...
        if (v !== lastP) {
            head.style.setProperty('--ft-grow', v);
            lastP = v;
        }
    }
    Scroll.add(update);
...
function initGarden() {
    ...
    function blade(x, y, ang, len, col) {
        const tx = x + Math.sin(ang * Math.PI / 180) * len,
            ty = y - Math.cos(ang * Math.PI / 180) * len;
    ...
    function buildLavender() {
        ...
        for (let i = 0; i <= rows; i++) {
            const t = i / rows,
                yy = spikeBot + (spikeTop - spikeBot) * t,
                spread = (1 - t) * 7 + 3;
        ...
        return {
            svg: wrap(w, h, leaves + stem + `<g class="head">${florets}</g>`),
            h
        };
    }
    ...
    function buildRidge(img, requireDark) {
        try {
            ...
            for (let s = 0; s < S; s++) {
                ...
                for (let y = 0; y < H; y++) {
                    const i = (y * W + col) * 4;
                    if (d[i + 3] > 200 && (!requireDark || (d[i] + d[i + 1] + d[i + 2]) / 3 < 72)) {
                        ridge = y;
                        break;
                    }
                }
    ...
    function refreshLand() {
        ...
        landH = lr.height;
        landW = lr.width; // land scales with the viewport (100vw / 150% / 340%)
        footerW = fr.width;
    ...
    function ridgeFrac(xf) {
        ...
        if (!prof) return 0.42;
        const t = imgFrac(xf) * (prof.length - 1),
            i = t | 0,
            f = t - i;
    ...
    function soilY(xf) {
        return landTop + ridgeFrac(xf) * landH + 7;
    }
    ...
    function ensureNightProf() {
        ...
        nightImg.addEventListener('load', () => {
            nightProf = buildRidge(nightImg, false);
            reroot();
        });
    ...
    if (document.body.classList.contains('night')) ensureNightProf();
        ...
        const nightWatch = new MutationObserver(() => {
            if (document.body.classList.contains('night')) {
                ensureNightProf();
                nightWatch.disconnect();
            }
        });
        nightWatch.observe(document.body, {
            attributes: true,
    ...
    let cursor = null,
        tipX = 44,
        tipY = 43; // spray tip (spout rose) within the 60-viewBox
    if (fine && !reduce) {
        cursor = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        cursor.setAttribute('viewBox', '0 0 60 60');
        cursor.setAttribute('width', '58');
        cursor.setAttribute('height', '58');
    ...
    // "click to grow" hint. Gated on the same `fine && !reduce` as the can —
    // a touch device has no hover state to reveal it with, and it would just
    // be a label stuck to the screen.
    let tip = null;
    if (cursor) {
        tip = document.createElement('div');
        tip.className = 'ft-tip';
        tip.textContent = 'Click to grow';
        tip.setAttribute('aria-hidden', 'true'); // decorative: the garden is not a control
    ...
    footer.addEventListener('pointermove', e => {
        ...
        if (tip) {
            // sits right of the pointer: the can's art hangs up-and-left of it
            // (58px box drawn from the spray tip), so this is the one side that
            // stays clear at any angle of approach.
            tip.style.transform = `translate(${e.clientX + 16}px, ${e.clientY + 2}px)`;
            const q = toFooter(e);
            tip.classList.toggle('on', !onControl(e.target) && nearPlant(q));
        }
        if (reduce) return;
        const now = performance.now();
    ...
    // the reach test the hint and the click share — same 80px radius to the
    // nearest point on a plant's stem, so the label appears exactly where a
    // click lands rather than approximating it
    const GROW_REACH = 80;

    function nearPlant(p) {
        return plants.some(pl => {
            const bx = p.r.width * pl.xPct / 100;
            const by = pl.baseY;
            const cy = clamp(p.y, by - pl.fullH * pl.g - 8, by);
            return Math.hypot(p.x - bx, p.y - cy) < GROW_REACH;
        });
    }
    const onControl = t => !!(t && t.closest && t.closest('.ft-head-btn, .ft-social'));
        ...
        plants.forEach(pl => {
            ...
            if (dist < 80) {
                ...
                if (pl.g < gCap) {
                    ...
                    pl.el.classList.remove('bloomed');
                    void pl.el.offsetWidth;
                    pl.el.classList.add('bloomed');
                }
            }
        });
    });
...
const pageLoader = (() => {
    ...
    function setLine(txt) {
        ...
        setTimeout(() => {
            lineEl.textContent = txt;
            lineEl.classList.remove('pl-line-swap');
        }, 300);
    }
